...
echo 'alias double="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/xianfeic/dmdb" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/xianfeic/dmdb'"
...
rm -rf "$HOME/.config/@double"
rm -rf "$HOME/.config/@double"
rm -rf "$HOME/.config/double-core"
